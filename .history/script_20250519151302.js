...
async function loadTasks() {
    ...
    const { data, error } = await supabase
        .from('tasks')
        .select('*, created_by_user_id(email), last_modified_by_user_id(email)')
        ...
        taskElement.innerHTML = `
            <div class="card-body">
                ...
                <div class="text-xs text-gray-500 mt-2">
                    Creada por: ${task.created_by_user_id} el ${new Date(task.created_at).toLocaleString()}
                    ${task.updated_at !== task.created_at ? `<br>Última modificación por: ${task.last_modified_by_user_id} el ${new Date(task.updated_at).toLocaleString()}` : ''}
                </div>
            </div>
        `;
